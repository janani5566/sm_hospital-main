...
class _FavouritesPageState extends State<FavouritesPage> {
  ...
  Future<void> InsertStudent() async {
    var response = await http.post(Uri.parse(baseUrl), headers: {
      ...
      "roomno": roomno.text,
      "status": status.text,
      "admitdate": admitdate.text,
      "dischargedate": dischargedate.text,
    ...
    if (response.statusCode == 200) {
      ...
      return showDialog(
        ...
        builder: (BuildContext context) {
          return AlertDialog(
            title: Text('Successfully Inserted..!!',
  ...
  Future<void> _getNewID() async {
    http.Response response =
        await http.get(Uri.parse('http://127.0.0.1:8000/sno/'));
    var data = json.decode(response.body);
    var lastID = data[data.length - 1]['ipno'];

    var parts = lastID.split('-');
    var currentYear = DateFormat('yy').format(DateTime.now());
    var number = int.parse(parts[2]);
    var ipno = '$currentYear-IP-${number + 1}';

    setState(() {
      perid.text = ipno.toString();
    });
  }
  ...
  Future<void> postData() async {
    ...
    http.Response response = await http.post(
        Uri.parse('http://127.0.0.1:8000/sno/'),
                            ...
                            children: [
                              ...
                              Row(
                                ...
                                children: [
                                  ...
                                  Padding(
                                    padding:
                                        const EdgeInsets.only(top: 19, left: 5),
                                    child: SizedBox(
                                      width: MediaQuery.of(context).size.width *
                                          0.18,
                                      child: OutlinedButton(
